...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    r"""type: (Tensor, float, float, float, float) -> Tensor
...
class PosEmbedRel(nn.Module):
    ...
    def __init__(self, block_size, win_size, dim_head, scale):
        ...
        self.height_rel = nn.Parameter(
            torch.randn(win_size * 2 - 1, dim_head) * scale)
    ...
    def forward(self, q):
        ...
        rel_logits_w = rel_logits_1d(
            q, self.width_rel, permute_mask=(0, 1, 3, 2, 4))

        # relative logits in height dimension.
        q = q.transpose(1, 2)
        rel_logits_h = rel_logits_1d(
            q, self.height_rel, permute_mask=(0, 3, 1, 4, 2))
...
class HaloAttn(nn.Module):
    ...
    def __init__(
        ...
        self.dim_head_qk = dim_head or make_divisible(
            dim_out * qk_ratio, divisor=8) // num_heads
        ...
        self.q = nn.Conv2d(dim, self.dim_out_qk, 1,
                           stride=self.block_stride, bias=qkv_bias)
        self.kv = nn.Conv2d(dim, self.dim_out_qk +
                            self.dim_out_v, 1, bias=qkv_bias)
    ...
    def forward(self, x):
        ...
        q = q.reshape(B * self.num_heads, self.dim_head_qk, -
                      1, num_blocks).transpose(1, 3)
        ...
        kv = F.pad(kv, [self.halo_size, self.halo_size,
        ...
        # fold
        out = out.reshape(-1, self.block_size_ds,
                          self.block_size_ds, num_h_blocks, num_w_blocks)
